...
from halina.asyncio_util_functions import gather_with_hard_deadline, wait_for_psce
...
class FileRapportService(ServiceNatsDependent):
    ...
    def __init__(self, **kwargs):
        ...
        self._collection_timeout: int = GlobalConfig.get(GlobalConfig.COLLECTION_TIMEOUT,
                                                         FileRapportService._DEFAULT_COLLECTION_TIMEOUT)
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    logger.exception(f"File rapport run for {now.date()} failed unexpectedly — "
                                     f"no file was saved, next run stays scheduled")
    ...
    async def _collect_data_and_save(self):
        ...
        # hard deadline: a single stuck reader must not block the rapport (and every following
        # night) forever — on timeout we still save a file built from the partial data
        completed = await gather_with_hard_deadline(coros, timeout=self._collection_timeout,
                                                    what="Night data collection (file rapport)")
        if not completed:
            logger.error(f"Night data collection did not finish within {self._collection_timeout}s — "
                         f"the file rapport will be saved from incomplete data")
