...
import argparse
import os
from simulator.data_publisher import DataPublisher
...
async def main(num_copies, host, port, telescopes, only_download, only_weather):
    ...
    weather_original_data = await data_publisher.read_json('weather')

    all_raw_copies = []
    all_zdf_copies = []
    all_download_copies = []

    weather = data_publisher.create_weather(weather_original_data, num_copies)
    await data_publisher.publish_weather(weather, host, port)
    if only_weather:
        logger.info("published only weather data")
        return
...
def run():
    ...
    parser.add_argument("--only_weather", action='store_true', help="If set, only publish weather data")
    ...
    asyncio.run(main(args.num_copies, args.host, args.port, telescopes, args.only_download, args.only_weather))
